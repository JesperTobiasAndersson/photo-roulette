...
export default function MusicQuizRoomScreen() {
  const { language, t, translateServerMessage, translateError } = useI18n();
  ...
  const run = async (key: string, fn: () => Promise<void>) => {
    setBusy(key);
    try { await fn(); await refresh(); } catch (error) { showAlert(copy.actionFailed, translateError(error)); } finally { setBusy(null); }
  };
  ...
  const banner: { icon: IconName; title: string; body?: string | null } | null = effectiveCompleted
    ...
    : inLobby
      ? isHost
        ...
        : { icon: "hourglass", title: copy.playerLobbyTitle, body: translateServerMessage(room.public_message) ?? copy.waitHost }
      : inQuestion
        ? isHost
        ...
        : inReveal
          ...
          : { icon: "hourglass", title: translateServerMessage(room.public_message) ?? copy.waitHost };
